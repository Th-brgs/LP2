...
public class Line extends Figure {
	...
	@Override
	   public String clickNaBorda(int mousex, int mousey) {
	    	...
	    	if ((mousex >= x-5) && (mousex <= (x+w+5))) {
	    		if (mousey >= y-5 && mousey <= y+5) {
	    			...
	    			if (mousey >= y-h-5 && mousey <= y-h+5) {
	    				return "s";
	    			}
	    		}
	    	}
	    	if ((mousey >=y-h+5) && (mousey <= (y-5))) {
	    		if (mousex >= x-5 && mousex <= x+w/4+5) {
	    			return "e";
	    		}
	    		else {
	    			if (mousex >= x+4*w/5-5 && mousex <= x+w-5) {
	    				return "d";
	    			}
	    		}
	    	}
	...
	public void resizemouseesquerda(int posx) {
        super.resizemouseesquerda(posx);
        pontosLine(x, y, w, h);
           
    }
	
	public void resizemousebaixo(int posy) {
        super.resizemousebaixo(posy);
        this.y = this.y + posy;
        pontosLine(x, y, w, h);
       }
	
	public void resizemousedireita(int posx) {
		super.resizemousedireita(posx);
        pontosLine(x, y, w, h);
    }
